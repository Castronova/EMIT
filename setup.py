...
APP_LOADER_DIR = os.path.join(MAC_DIR, 'Dist', "EMIT.app")
# Location of Windows files
APP_LOADER_FILE = os.path.join(BASE_DIR, "EMIT.py")
...
ICE_SCRIPT = os.path.join(MAC_DIR, "EMIT_setup.packproj")
...
def obtain_exe_filename(console=False):
    if console:
        return "{app}_{version}_{os}_{arch}_{type}".format(app="EMIT",
        version="1.0", os=sys.platform, arch='x86_64', type= "console")
    else:
        return "{app}_{version}_{os}_{arch}".format(app="EMIT",
        version="1.0", os=sys.platform, arch='x86_64')
...
def mac_pyinstaller(Name = None, File = None):
    try:
        ...
        print APP_DIR
        os.system("cp /Users/lil9porkchop/miniconda/envs/emit/lib/libwx_osx_cocoau-3.0.0.0.0.dylib %s" % os.path.join(APP_DIR, "Contents", "MacOS"))
...
def main():
    ...
    if sys.platform == 'win32':
        ...
        if run_pyinstaller('EMIT', os.path.join(BASE_DIR, "EMIT.py")):
        ...
        if mac_pyinstaller('EMIT', os.path.join(BASE_DIR, "EMIT.py")):
